/**
 * Novelette Page Controller
 * Enhanced novelette rendering with chapter expansion.
 * 
...
/**
 * Load all novelette content
 */
export async function loadNovelette() {
    await loadEnhancedBackstory();
    events.emit('novelette:loaded');
}
...
export async function loadEnhancedBackstory() {
    ...
    container.innerHTML = '<div class="loading-spinner">Loading novelette...</div>';
    ...
    try {
        ...
        console.error('Failed to load novelette:', error);
        container.innerHTML = '<p class="error-message">Failed to load novelette.</p>';
    }
}
...
events.on('page:change', ({ page }) => {
    if (page === 'novelette' && !backstoryData) {
        loadNovelette();
    }
});
